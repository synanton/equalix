...
- Tk(t)Tk(t) — accumulated virtual scheduling time.
...
Fk(t2)=Fk(t1)+Sk(t1,t2)−Tk(t1,t2)Fk(t2)=Fk(t1)+Sk(t1,t2)−Tk(t1,t2)

where:

- SkSk — tasks entering the in-flight state.
- TkTk — tasks leaving the in-flight state.

Equivalently:

ΔFk=ΔSk−ΔTkΔFk=ΔSk−ΔTk
...
EA=10%EA=10%EB=20%EB=20%EC=70%EC=70%
...
Ek(t)=[Fk(t)<Qk]Ik(t)=p(t)F^k(t)wkAx(t)=λWx(t)Px(t)=Tk(t)+Ik(t)−Ax(t)x∗=arg⁡min⁡x(Px,arrivalx,idx)Tk←Tk+1wkEk(t)Ik(t)Ax(t)Px(t)x∗Tk=[Fk(t)<Qk]=p(t)wkF^k(t)=λWx(t)=Tk(t)+Ik(t)−Ax(t)=argxmin(Px,arrivalx,idx)←Tk+wk1
